...
function placeBet(){
  ...
  const allIn=stk>=G.bal; // capture BEFORE deducting
  G.bal-=stk;
...
function simBet(sels,stk,o,betId,allIn,legs){
  ...
  const TIMEOUT_MS=120000; // force-resolve after 2 minutes max
  const intervalId=setInterval(()=>{
    const allSports=Object.values(G.evts).flat();
    // Event is done if: finished flag set, or startTime=FT, or event no longer exists (new round), or timed out
    const allDone=sels.every(s=>{
      const ev=allSports.find(e=>e.id===s.eid);
      if(!ev)return true; // event replaced by new round — resolve
    ...
    if(!betRec)return; // already resolved (e.g. cashed out)
    betRec.intervalId=null;
    const win=Math.random()<Math.min(0.88,1/o*0.92);
    const pay=win?parseFloat((stk*o).toFixed(2)):0;
...
function goTab(t,btn){
  ...
  ['sportsbook','live','leaderboard','history','loans','profile','redeem'].forEach(x=>document.getElementById('tab-'+x).classList.toggle('hidden',x!==t));
  if(t==='leaderboard')renderLB();
  if(t==='history')renderHistory();
  if(t==='live')renderLive();
  if(t==='loans')renderBank();
  if(t==='profile')renderProfile();
  if(t==='redeem')renderRedeem();
}
