...
function genRaces(type){
  ...
  for(let r=0;r<4;r++){
    ...
    // 30% chance one runner is scratched
    if(Math.random()<0.3){
      const scratchIdx=ri(0,runners.length-1);
      runners[scratchIdx].scratched=true;
    }
    G.evts[type].push({id:type[0]+r,sport:type,title:pick(races),info:n+' runners · '+(type==='horses'?ri(1,4)+'m '+ri(0,7)+'f':'480m'),runners,isLive:false,startTime:r===1?ri(5,30)+'min':ri(1,4)+'h'});
  }
}
...
function genMMA(){
  ...
  for(let i=0;i<pool.length-1&&i<12;i+=2){
    ...
    G.evts.mma.push({id:'m'+i,sport:'mma',event:pick(MEVT),f1:f1.n,f1ovr:f1.o,f2:f2.n,f2ovr:f2.o,
      ...
      isLive:false,round:0,startTime:ri(1,8)+'h'});
  }
}
...
function genOlympic(){
  G.evts.olympic=[];
  const events=shuf(OLYMPIC_EVENTS).slice(0,3);
  events.forEach((evName,ei)=>{
    const n=6;
    const runners=shuf(OLYMPIC_RUNNERS).slice(0,n).map((nm,i)=>{
      const ov=ri(72,96);
      const wo=parseFloat(Math.max(1.2,(n-ov/96*n+1)*rnd(0.85,1.3)).toFixed(2));
      return {name:nm,ovr:ov,num:i+1,winOdds:wo,placeOdds:parseFloat((wo*.33).toFixed(2)),ew:parseFloat((wo*.38).toFixed(2))};
    });
    G.evts.olympic.push({
      id:'ol'+ei,sport:'olympic',title:evName,
      info:n+' athletes',runners,
      isLive:false,startTime:ei===0?ri(5,30)+'min':ri(1,4)+'h'
    });
  });
}
function generateAll(){genFootball();genBasketball();genRaces('horses');genRaces('dogs');genMMA();genTennis();genCricket();genF1();genOlympic();}
...
const SPORT_FINISH={
  ...
  cricket:    ev=>(ev.hWkts||0)>=7||parseFloat(ev.overs||0)>=20,
  f1:         ev=>ev.lap>=ev.totalLaps&&typeof rProg!=='undefined'&&rProg[ev.id]&&rProg[ev.id].every(p=>p>=100),
  olympic:    ev=>typeof rProg!=='undefined'&&rProg[ev.id]&&rProg[ev.id].some(p=>p>=100),
};
...
function newRound(sport){
  ...
  if(sport==='horses'||sport==='dogs'||sport==='f1'||sport==='olympic'){
  ...
  else if(sport==='olympic')genOlympic();
  renderAll();
}
