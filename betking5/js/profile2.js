  ...
  document.getElementById('redeemCon').innerHTML = `
    <div class="profile-card">
      <div style="text-align:center;margin-bottom:20px">
        ...
        <div style="font-family:'Rajdhani',sans-serif;font-size:18px;font-weight:700">Secret Redeem Code</div>
        <div style="font-size:11px;color:var(--txt3);margin-top:4px">Enter a code to unlock special perks</div>
      </div>
      <div class="inp-row">
        <div class="inp-lbl">Code</div>
        <input class="pinp" id="redeemInput" placeholder="Enter code here..." maxlength="30"
          style="letter-spacing:2px;text-transform:uppercase"
          oninput="this.value=this.value.toUpperCase()"
          onkeydown="if(event.key==='Enter')submitRedeemCode()">
      </div>
      <button onclick="submitRedeemCode()"
        style="width:100%;margin-top:10px;background:rgba(240,180,41,.15);border:1px solid rgba(240,180,41,.4);
               color:var(--gold);font-family:'Rajdhani',sans-serif;font-size:14px;font-weight:700;
               padding:11px;border-radius:8px;cursor:pointer;letter-spacing:1px">
        🔓 REDEEM
      </button>
      <div style="margin-top:18px;padding-top:14px;border-top:1px solid var(--bdr)">
        <div style="font-size:10px;color:var(--txt3);margin-bottom:6px">AD STATUS ON THIS DEVICE</div>
        <div style="display:flex;align-items:center;gap:8px">
          <div style="width:10px;height:10px;border-radius:50%;background:${adsBlocked ? 'var(--red)' : 'var(--grn)'}"></div>
          <span style="font-family:'Rajdhani',sans-serif;font-size:13px;font-weight:700;color:${adsBlocked ? 'var(--red)' : 'var(--grn)'}">
            ${adsBlocked ? 'Ads Blocked 🚫' : 'Ads Active ✅'}
          </span>
        </div>
