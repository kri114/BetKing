...
function renderEvents(){
  ...
  try{
    ...
    else if(G.sport==='olympic')con.innerHTML=evts.map(rOlympic).join('');
  }catch(err){
...
function rRace(e){
  ...
  const rows=e.runners.map((r,i)=>{const scratch=r.scratched;const numBg=scratch?"#555":RCOLS[i];const nameStyle=scratch?"text-decoration:line-through;color:var(--txt3)":"";const ovrHtml=scratch?"<span style=\"font-size:9px;color:var(--red)\">❌ DNS</span>":`<span class=\"rovr-b\" style=\"background:${r.ovr>=85?"rgba(0,212,122,.12)":r.ovr>=75?"rgba(240,180,41,.12)":"rgba(255,48,80,.12)"};color:${r.ovr>=85?"var(--grn)":r.ovr>=75?"var(--gold)":"var(--red)"}\">${r.ovr}</span>`;const scratchLabel=scratch?" <span style=\"color:var(--red);font-size:9px\">SCRATCHED</span>":"";return `<div class="rrow${scratch?" scratched":""}"><div class="rnum" style="background:${numBg}">${i+1}</div><div class="rname" style="${nameStyle}">${r.name}${scratchLabel}</div>${ovrHtml}</div>`;}).join("");
