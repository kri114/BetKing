...
function initVP(ev){
  ...
  } else if(ev.sport==='olympic'){
    vP=(ev.runners||[]).map((r,i)=>{
      return {x:0,y:(i+1)/(ev.runners.length+1),tx:0,ty:(i+1)/(ev.runners.length+1),c:RCOLS[i%8]||'#4FC3F7',name:r.name.split(' ')[0],num:r.num,team:i};
    });
  } else if(ev.sport==='f1'){
...
function tickV(ev){
  ...
  if(sp==='football'&&ev.minute<90){
    ...
    rProg[ev.id].forEach((_,i)=>{if(ev.runners[i].scratched)return;rProg[ev.id][i]=Math.min(100,rProg[ev.id][i]+rnd(.4,2.8)*(0.7+ev.runners[i].ovr/200));});
    if(Math.random()<.1){const lead=[...rProg[ev.id].keys()].sort((a,b)=>rProg[ev.id][b]-rProg[ev.id][a])[0];addC('—',ev.runners[lead].name+' — '+pick(RCOMM));}
  } else if(sp==='olympic'){
    if(!rProg[ev.id])rProg[ev.id]=ev.runners.map(()=>rnd(0,3));
    rProg[ev.id].forEach((_,i)=>{rProg[ev.id][i]=Math.min(100,rProg[ev.id][i]+rnd(0.8,2.5)*(0.7+ev.runners[i].ovr/200));});
    if(Math.random()<.1){const lead=[...rProg[ev.id].keys()].sort((a,b)=>rProg[ev.id][b]-rProg[ev.id][a])[0];addC('—',ev.runners[lead].name+' — '+pick(['Pulling ahead!','Setting the pace!','Making a move!','Huge sprint finish!','Neck and neck!','Breaking away!']));}
  } else if(sp==='mma'){
    ...
    if(Math.random()<.06){
      ...
      if((ev.p1Games||0)>=6&&(ev.p1Games||0)-(ev.p2Games||0)>=2){
        ...
        addC('Set '+(ev.p1Sets+ev.p2Sets),ev.p2.split(' ').pop()+' wins the set!');
      }
    }
...
function drawFrame(ev){
  ...
  else if(sp==='olympic')drawRaceTrack(cv,ev);
...
function openViewer(eid,e){
  ...
  const all=[...(G.evts.f1||[]),...G.evts.football,...G.evts.basketball,...G.evts.horses,...G.evts.dogs,...G.evts.mma,...(G.evts.tennis||[]),...(G.evts.cricket||[]),...(G.evts.olympic||[])];
  ...
  const cvH=sp==='football'?330:sp==='basketball'?280:isRace||sp==='olympic'?220:isF1||isTennis||isCricket?240:260;
  const title=sp==='olympic'?`🏃 ${ev.title}`:isTeam?`${sp==='football'?'⚽':'🏀'} ${ev.home} vs ${ev.away}`:isMMA?`🥊 ${ev.f1} vs ${ev.f2}`:isF1?`🏎️ ${ev.circuit}`:isTennis?`🎾 ${ev.p1} vs ${ev.p2}`:isCricket?`🏏 ${ev.home} vs ${ev.away}`:`${sp==='horses'?'🐎':'🐕'} ${ev.title}`;
